...
**它不是作者身份的证据。** 在留出数据上，**9.8% 的真实已发表英文论文窗口、4.6% 的真实已发表中文论文
...
| 特征数 | 45 | 48 |
| 窗口数（训练/验证/测试，人+机） | 720+789 / 240+264 / 240+267 | 711+770 / 238+264 / 237+267 |
| 训练轮数 → 保留 | 900 → **163** | 900 → **138** |
| 验证损失最低点 | 499 | 387 |
...
499 轮（英文）/ 387 轮（中文）触底，之后回升。

但发布的模型是**第 163 / 138 轮**：验证损失首次落入最低点**一倍标准误**之内的那一轮。取精确的最低点
